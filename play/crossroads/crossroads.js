...
// ── 삽입 시나리오 2건(2026-07 인물 보강과 함께 추가) ───────────
// 배열 마지막 'today' 앞에 시간순으로 끼워 넣는다: 1907(정미의병) →
// 1938(문화로 저항하기) → today. 두 신설 카드 모두 이번에 추가한
// 인물카드(신돌석·윤희순·최승희·박차정·전형필)로 실제 역사를 연결한다.
SCENARIOS.splice(SCENARIOS.length - 1, 0,
  ...
  },
  {
    ...
    options: [
      { text: '조선의 전통을 현대 예술의 언어로 재창조해 세계 무대로 가져간다', axes: { p: 2, s: 1 }, frag: '나는 억압 속에서도 우리 것을 세계의 무대에 올렸다' },
    ...
    reveal: '최승희는 조선 춤을 현대무용으로 재창조해 파리와 뉴욕 무대에서 세계가 아는 최초의 한국 예술가가 됐고, 박차정은 근우회의 여성운동가에서 조선의용대의 전사로 나섰습니다. 같은 시기 전형필은 전 재산을 들여 훈민정음 해례본 등 문화재를 사들여 지켜냈습니다 — 총이 아닌 방식의 저항이 셋이나 있었습니다.',
    link: '../../map.html?event=person_1943_03', link_label: '전형필 — 지도에서 보기',
  }
);
