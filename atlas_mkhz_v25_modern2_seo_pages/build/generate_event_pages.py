...
def render_html(card, slug, breadcrumb, related_cards, slug_map):
    ...
    hero_url = f"{SITE_ROOT}/{card['hero_image']}" if card['hero_image'] else f'{SITE_ROOT}/assets/images/og-image.jpg'
    ...
    if related_cards:
        ...
        for rc in related_cards:
            ...
            if not r_slug:
                continue
            items.append(f'<li><a href="/event/{r_slug}">{esc(rc["title_ko"])} ({rc["year"]})</a></li>')
        if items:
