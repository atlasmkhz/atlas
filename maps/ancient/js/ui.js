...
// ── 나라 필터 (고구려/백제/신라/가야/발해/후백제/태봉/고려) ──
// 여러 나라가 같은 시공간에 동시에 있었다는 이 지도 특유의 레이어라,
// 사건/인물/정책과는 별도 그룹으로 둔다. app.js의 NATION_LABEL 키와
// checkbox id(layerNation_{key})가 항상 일치해야 한다.
if (typeof NATION_LABEL === 'object') {
  Object.keys(NATION_LABEL).forEach(nid => {
    const el = document.getElementById('layerNation_' + nid);
    if (!el) return;
    el.addEventListener('change', function() {
      if (window.trackFilterChange) window.trackFilterChange('나라:' + NATION_LABEL[nid], this.checked);
      if (typeof isSearchActive === 'function' && isSearchActive()) return;
      if (typeof renderCurrentChapter === 'function') renderCurrentChapter();
    });
  });
