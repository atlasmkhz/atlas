...
const MARKER_SVG = {
  ...
  // 대형참사 — 경고 삼각형(사고·재난, 국가폭력에 의한 학살과는 구분)
  disaster: '<path d="M12 3.5l9.5 16.5H2.5z" stroke="currentColor" stroke-width="1.8" fill="none" stroke-linejoin="round"/><path d="M12 9.5v5" stroke="currentColor" stroke-width="2" stroke-linecap="round"/><circle cx="12" cy="17" r="1.1" fill="currentColor"/>',
...
// nationColor가 있으면 아이콘 바깥에 나라색 고리(ring)를 두른다 — 유형색
// (아이콘 자체)은 "무슨 일이 있었나", 이 고리색은 "어느 나라 일이었나"를
// 나타내는 이중 인코딩이다. 여러 나라가 같은 시공간에 동시에 있었다는
// 이 지도 특유의 철학을 마커 하나로 바로 보여주기 위함.
function buildMarkerHtml(type, { color, opacity = 1, pulse = false, nationColor = null } = {}){
  const inner = getMarkerSvgInner(type);
  const pulseDiv = pulse
    ? `<span class="atlas-marker-pulse" style="background:${color};"></span>`
    : '';
  const ringDiv = nationColor
    ? `<span class="atlas-nation-ring" style="border-color:${nationColor};"></span>`
    : '';
...
function makeMarkerIcon(type, opts){
  return L.divIcon({
    ...
    iconSize: [34, 34],
    iconAnchor: [17, 17],   // 중심 정렬 (나라색 고리가 아이콘보다 커서 앵커도 함께 키움)
    popupAnchor: [0, -15],
