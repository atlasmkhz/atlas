...
// 있다(예: 이 지도(근현대)에서 "안종삼"을 쳤는데 그 카드는 사실 여기
// 있다 — 자기 자신은 아래에서 제외됨). data/search_index.js가 미리
// 만들어둔 경량 통합 인덱스(SEARCH_INDEX, 전역)에서 같은 검색어로 매칭되는
// 다른 지도의 항목을 찾아 "◯◯ 시대에 있음" 안내+링크만 보여준다. 이
// 지도 화면에 마커를 그리지는 않는다 — 다른 지도는 완전히 독립된
// 페이지라 여기서 미리보기를 그릴 방법이 없기 때문이다.
const THIS_MAP_KEY = 'modern2';
...
function renderSearchResults(results){
  ...
  results.forEach(e=>{
    ...
    if(e.area){
      ...
      circle.on('click', ()=>{ if(typeof navigateToEvent === 'function') navigateToEvent(e.id); else if(window.openInfoPanel) openInfoPanel(popupHtml(e)); });
      layers.push(circle);
    }
    ...
    m.on('click', ()=>{
      // 검색 결과 클릭 시 해당 사건의 연도로 슬라이더/지도를 이동한 뒤
      // 정보창을 연다(navigateToEvent가 처리) — 검색만으로는 슬라이더가
      // 그대로였던 문제(2026-07-25 두목님 피드백)를 여기서 고친다.
