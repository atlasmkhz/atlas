...
// ── BCE 연도 표기 ── 선사시대는 기원전 연도가 대부분이라(-700000 =
// 기원전 70만 년), year 값을 그대로 화면에 찍으면 "-700000년"처럼
// 보인다. 이 헬퍼로 통일해서 "기원전 70만"/"기원전 300" 형태로 표시한다.
// 아주 큰 수(구석기·신석기)는 "만" 단위로 축약해 가독성을 높인다.
function formatYearShort(y){
  ...
  if (abs >= 10000) {
    ...
    return `기원전 ${rounded % 1 === 0 ? rounded.toFixed(0) : rounded}만`;
  }
  return `기원전 ${abs}`;
}
...
// 선사시대는 여섯 블록(신화시대~고조선)이 사건 밀도·성격이 완전히
// 달라 "왕 파일 하나씩"이 아니라 "블록 파일 하나씩"을 쓴다(고대 지도의
// blocks.js와 같은 방식). 새 블록의 데이터 파일을 추가할 때는:
// (1) data/blockN.js 파일 생성, (2) index.html에 <script> 추가,
// (3) 아래 배열에 그 EVENTS_* 변수명 추가.
const DATA = {};
[
...
// selectReign(0)으로 첫 챕터(신화시대)부터 시작한다(아래 참고).
window.onload = () => {
  requestAnimationFrame(() => {
    ...
    setTimeout(() => {
      ...
      // 범위(기원전 100만~37) 밖이면 가장 가까운 경계 연도로 clamp한다.
      ...
      // 기본 진입: 첫 챕터(신화시대)부터 시작. 시대개요 자동 오픈은
