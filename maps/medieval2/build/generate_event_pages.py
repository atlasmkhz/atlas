# -*- coding: utf-8 -*-
"""
generate_event_pages.py (maps/medieval2 전용)
중세2(조선, 1392~1875) Event 데이터를 읽어 maps/medieval2/event/{slug}.html
정적 페이지를 생성한다.
...
스크립트로 분리했다. 또한 근대 생성기가 의존하는 generate_slugs.py /
era_breadcrumb.py는 이 프로젝트 배포본에 포함돼 있지 않으므로(별도
빌드 환경에만 존재), 이 스크립트는 슬러그·브레드크럼 로직을 자체
내장해 완전히 독립적으로 실행된다 — 이 폴더(maps/medieval2/build/) 밖의
어떤 파일에도 의존하지 않는다.
...
- 지도 하이드레이션(mapShell.js)은 이 지도의 shellUrl
  ('/maps/medieval2/index.html')을 명시적으로 넘겨서 호출한다 — 근대
  이벤트 페이지가 쓰는 기본값(/map.html)과는 다른 지도이기 때문이다.

사용법:
    cd maps/medieval2/build
...
MAP_DIR = os.path.dirname(BUILD_DIR)               # .../maps/medieval2
PROJECT_ROOT = os.path.dirname(os.path.dirname(MAP_DIR))  # 사이트 루트

SITE_ROOT = 'https://atlas.mkhz.kr'
MAP_URL_PATH = '/maps/medieval2/index.html'        # 이 지도의 shellUrl
...
def render_html(card, slug, breadcrumb, related_cards, slug_map):
    url = f'{SITE_ROOT}/maps/medieval2/event/{slug}'
    ...
    if related_cards:
        ...
        for rc in related_cards:
            ...
            if not r_slug:
                continue
            items.append(f'<li><a href="/maps/medieval2/event/{r_slug}">{esc(rc["title_ko"])} ({rc["year"]})</a></li>')
        if items:
