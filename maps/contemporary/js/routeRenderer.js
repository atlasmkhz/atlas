...
(function () {
  ...
  const WP_TYPE_COLOR = {
    ...
    party_dp: '#2255a4',     // 파랑 — 더불어민주당 계보 (대한민국 정당사 루트 전용)
    party_ppp: '#b53a3a',    // 빨강 — 국민의힘 계보 (대한민국 정당사 루트 전용)
  };
...
<div class="route-waypoint-popup">
  ...
  function buildRoutePanelHtml(route) {
    const wpRows = route.waypoints.map((wp, i) => {
      ...
      // wp.archive_post가 있으면 자료실 해당 글로 이어지는 링크를 붙인다
      // (2026-07-12 교주와 권력 루트 추가로 도입 — 선사시대 환단고기
